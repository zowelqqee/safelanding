...
function parseInput(body: unknown): CityModelInput | null {
  ...
  const countryId = stringValue(body.countryId);
...
export async function POST(request: Request) {
  ...
  if (!input) {
    return Response.json({ error: "Invalid city prediction request" }, { status: 400 });
  }
  ...
  try {
    ...
    console.warn("City model unavailable; using heuristic city matcher.", error);
    return Response.json({
      source: "heuristic",
